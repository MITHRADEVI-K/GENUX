...
// Simple logging/tracking for Gemini calls
const LOG_DIR = path.join(__dirname, '..', 'logs')
try { if (!fs.existsSync(LOG_DIR)) fs.mkdirSync(LOG_DIR, { recursive: true }) } catch (e) {}
const LOG_FILE = path.join(LOG_DIR, 'gemini-calls.csv')
if (!fs.existsSync(LOG_FILE)) {
  try { fs.writeFileSync(LOG_FILE, 'timestamp,success,latencyMs,confidence,model,endpoint,note\n', 'utf8') } catch (e) {}
}

function logGeminiCall({ success = false, latencyMs = 0, confidence = 0, model = '', endpoint = '', note = '' } = {}) {
  const ts = new Date().toISOString()
  const line = `${ts},${success ? 1 : 0},${Number(latencyMs).toFixed(0)},${Number(confidence).toFixed(3)},"${model}","${endpoint}","${String(note).replace(/\"/g, "'")}"\n`
...
async function callGeminiLive(prompt) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logGeminiCall({ success: false, latencyMs: latency, confidence: 0, model: GEMINI_MODEL, endpoint: url, note: `status ${res.status}` })
      throw new Error(`Gemini API error: ${res.status} ${txt}`)
    }
    const json = await res.json()
    const generatedText = json.candidates?.[0]?.content?.parts?.[0]?.text || ''
    // Note: Gemini doesn't provide a standardized 'confidence' in this API; return a heuristic
    const conf = json.candidates?.[0]?.metadata?.confidence || 0.85
    logGeminiCall({ success: true, latencyMs: latency, confidence: conf, model: GEMINI_MODEL, endpoint: url })
    return { interpretation: generatedText, confidence: conf }
  } catch (err) {
    const latency = Date.now() - start
    logGeminiCall({ success: false, latencyMs: latency, confidence: 0, model: GEMINI_MODEL, endpoint: url, note: err?.message || String(err) })
    throw err
  }
}
  ...
  async analyze(structuredPrompt) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        logGeminiCall({ success: false, latencyMs: latency, confidence: 0, model: this.endpoint, endpoint: this.endpoint, note: `status ${res.status}` })
        return this._stubAnalyze(structuredPrompt)
      }
      const json = await res.json()
      ...
      logGeminiCall({ success: true, latencyMs: latency, confidence: Number(confidence) || DEFAULT_CONFIDENCE, model: this.endpoint, endpoint: this.endpoint })
      ...
      logGeminiCall({ success: false, latencyMs: -1, confidence: 0, model: this.endpoint, endpoint: this.endpoint, note: err?.message || String(err) })
      return this._stubAnalyze(structuredPrompt)
    }
  }
